...
{
    /// <summary>
    /// Stock <c>MapManager.HeroControllerOnUpdate</c> sets <c>hasMapIcon</c> from <c>TryGetMapPosition</c> first.
    /// That is false while <c>GameMap</c> is unavailable (often until the map has been opened), so the server
    /// never gets <c>HasIcon</c> even when compass rules say broadcast — e.g. joiner notifies before host has
    /// resolved map state. This prefix replaces the callback so the <i>network</i> flag follows
    /// <c>AlwaysShowMapIcons</c> / compass settings only; position packets still require a resolvable map position.
    /// (Older SSMP builds also had an inverted compass branch; this replaces the full method either way.)
    ///
        ...
        private static class MapBroadcastReflect
        {
            ...
            internal static bool TryBuild(Type mapManagerType)
            {
                ...
                try
                {
                    ...
                    var ssmpVecType = SsmpReflect.MathVector2;
