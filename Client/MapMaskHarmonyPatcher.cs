...
namespace HornetCloakColor.Client
{
    ...
    ///   <item><c>InventoryWideMap.UpdatePositions</c> / <c>PositionIcon</c> — local player icon tint on the wide
    ///         "overall" map only (SSMP does not show remote players there by design).</item>
    ...
    internal static class MapMaskHarmonyPatcher
    {
        ...
        internal static void Apply()
        {
            ...
            ApplyVanillaMapTintPatches(harmony);

            // Mono-player / no SSMP: skip SSMP assembly patches entirely (no wasted reflection).
            if (SSMPBridge.IsAvailable)
                ApplySsmpCompanionPatches(harmony);
        }

        /// <summary>Game map + wide inventory map mask tint only — no SSMP types referenced.</summary>
        private static void ApplyVanillaMapTintPatches(Harmony harmony)
        {
        ...
        private static void TryPatchSsmpCreatePlayerIcon(Harmony harmony)
        {
            var mapManagerType = SsmpReflect.MapManager;
            var vec2Type = SsmpReflect.MathVector2;
        ...
        private static void InventoryWideMap_UpdatePositions_Postfix(object __instance)
        {
            ...
            try
            {
                ...
                LocalMapMaskTint.RefreshObject(iconTransform.gameObject, plugin.ColorConfig.EffectiveColor);
            }
            catch (Exception ex)
            {
        ...
        private static void InventoryWideMap_PositionIcon_Postfix(object __instance, Transform icon, bool isActive)
        {
            ...
            try
            {
                ...
                LocalMapMaskTint.RefreshObject(icon.gameObject, plugin.ColorConfig.EffectiveColor);
            }
            catch (Exception ex)
            {
